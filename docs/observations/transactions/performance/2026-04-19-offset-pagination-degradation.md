...
Resolvido na Onda D (commits `6570010`, `318eec1`), validado pelo usuario. Adotada paginacao cursor (Next/Previous apenas, sem page-jump):
...
**Verificacao**: testes unit `firebase-cursor-list.test.ts` (limit+1, hasNext/trim, cursor, withCount) e `paginate-transactions.test.ts` (browse delega ao cursor e repassa o cursor). `ts-check` limpo; suite verde. Navegacao Next/Prev em volume real validada pelo usuario.
