...
export const applyTransactionToReport = ({ report, transaction, direction }: IProps): IReportBase => {
  ...
  if (isDeposit) {
    updated.totalIncome = Math.max(0, updated.totalIncome + amount * direction);
  } else {
    updated.totalExpenses = Math.max(0, updated.totalExpenses + amount * direction);
  }
  ...
  for (const categoryId of transaction.categoryIds) {
    ...
    const newValue = Math.max(0, current + amount * direction);
    if (newValue === 0) {
  ...
  if (isDeposit) {
    monthEntry.income = Math.max(0, monthEntry.income + amount * direction);
  } else {
    monthEntry.expenses = Math.max(0, monthEntry.expenses + amount * direction);
  }
