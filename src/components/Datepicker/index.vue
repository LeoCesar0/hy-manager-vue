  ...
  <UiPopover v-slot="{ close }">
    <div :class="cn('w-full max-w-[250px]', props.class)" v-bind="{
      ...
      <div class="flex items-center">
        ...
        " variant="outline" size="icon" class="rounded-l-none border-l-0">
          <Cross2Icon />
        </UiButton>
      </div>
    </div>
    <UiPopoverContent class="w-auto p-0">
      <Calendar :model-value="modelValue"
        @update:model-value="(value) => { emit('update:modelValue', value); close(); }"
        :default-placeholder="defaultPlaceholder" layout="month-and-year" initial-focus />
    </UiPopoverContent>
  </UiPopover>
</template>
