<script lang="ts">
import type { ICategoryBudget } from "~/@schemas/models/budget";

// Shared payload shape produced by the form. Consumers (the reports dialog and
// the Objetivos page) type their `onSave` against this so the contract lives in
// one place.
export type IBudgetFormPayload = {
...
const categoryBudgets = ref<
  { categoryId: string; type: "expense" | "deposit"; amount: string }[]
>([]);

// Hydrate from the budget snapshot on mount. Parents remount via `:key`
// (dialog: each open; page: on account change / after save) to re-hydrate, so
// an abandoned edit never leaks into the next open or the next account.
...
const selectableCategoriesFor = (currentId: string) => {
  ...
  const list = current
    ? [current, ...availableCategories.value]
...
// Parses the raw input strings into the persisted shape. Zero/blank amounts are
// dropped; a non-positive overall limit/goal becomes null (treated as "unset").
const getPayload = (): IBudgetFormPayload => {
